<?php
   ...
   //Alert ...In this code ... I make a javascript and payment and insert data success.
   // Problem .... When javascript is complete then after when double click on submit button after payment process display ...
   // generate bill process in running mode
?>
...
$(document).ready(function () {
    $('#payNow').click(function (e) {
        ...
        initiateRazorpayPayment(); // Call function to initiate payment
    });
});
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (responseData.exists) {
                    // Email exists, proceed with checking event details
                    checkEventDetails();
                } else {
                    // Email does not exist, display alert and redirect to login page
                    alert("Please login first.");
                    window.location.href = "e_login.php"; // Replace 'e_login.php' with your actual login page URL
                }
            },
...
function checkEventDetails() {
    var billing_place = $('#placeid').val();
    var billing_date = $('#datepicker').val();
    var billing_time = $('#time').val();

    $.ajax({
        url: 'check_event.php',
        type: 'POST',
        data: { place: billing_place, date: billing_date, time: billing_time },
        success: function (response) {
            if (response.trim() === "exists") {
                alert("Event is already booked for the selected place, date, and time.");
            } else if (response.trim() === "not_exists") {
                // Event is not booked, proceed with initiating Razorpay
                initiateRazorpay();
            } else {
                alert("Error checking event details. Please try again later.");
            }
        },
        error: function (xhr, status, error) {
            alert("Error checking event details. Please try again later.");
...
function initiateRazorpay() {
    ...
    var billing_email = $('#useremail').val();
    var billing_lname = $('#ulname').val();
    var billing_address = $('#uadd').val();
    var billing_state = $('#stateid').val();
    var billing_city = $('#cityid').val();
    var billing_area = $('#areaid').val();
    var billing_place = $('#placeid').val();
    var billing_date = $('#datepicker').val();
    var billing_time = $('#time').val();
    var billing_rent = $('#rprice').val();
    var billing_dectype = $('#dectypeid').val();
    var billing_member = $('#memnum').val();
    var billing_decprice = $('#decpriceid').val();
    var billing_event = $('#event').val();
    var billing_food = $('#food').val();
    var billing_manger = $('#mname').val();
    var billing_memail = $('#memail').val();
    var billing_mcontact = $('#mcontact').val();
    var billing_mplace = $('#mplace').val();
    var billing_rprice = $('#rprice2').val();
    var billing_dprice = $('#decpriceid2').val();
    var tPrice = parseFloat($('#tprice').val()); // Corrected to use parseFloat for decimal values
    var billing_discount = $('#billing_discount').val();

    // Calculate payAmount (assuming 50% payment)
    var payAmount = tPrice / 2 || 0;

    // AJAX request to calculate discount
    var formDataDiscount = {
        billing_email: billing_email,
        billing_place: billing_place,
        payAmount: payAmount,
        tPrice: tPrice // Send total price for accurate discount calculation
    };
    ...
    $.ajax({
        ...
        success: function(response) {
            console.log('Discount calculated successfully:', response);
            var discountResponse = JSON.parse(response); // Parse the JSON response
            var discount = discountResponse.discount;

            // Replace "YOUR_RAZORPAY_KEY" with your actual Razorpay test key
            var options = {
                ...
                "handler": function(response) {
                    if (response.error) {
                        ...
                        // Insert data into book_form table
                        var formDataInsert = {
                            order_id: order_id,
                            ...
                            billing_rent: billing_rent,
                            billing_dectype: billing_dectype,
                            billing_member: billing_member,
                            billing_decprice: billing_decprice,
                            billing_event: billing_event,
                            billing_food: billing_food,
                            billing_manger: billing_manger,
                            billing_memail: billing_memail,
                            billing_mcontact: billing_mcontact,
                            billing_mplace: billing_mplace,
                            billing_rprice: billing_rprice,
                            billing_dprice: billing_dprice,
                            payAmount: payAmount,
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                // Redirect to payment-success.php with necessary data
                                window.location.href = "payment-success.php?" + $.param(formDataInsert);
                            },
                            error: function(xhr, status, error) {
                                console.error('Error inserting data:', error);
                                window.location.href = "payment-failed.php"; // Redirect to failure page
                            }
                        });
                    }
                },
        ...
        error: function(xhr, status, error) {
            ...
            alert("Error calculating discount. Please try again later.");
        }
    });
}
...
<form id="payForm" action="form.php" method="POST" onsubmit="validateFormAndInitiatePayment(event)">
      <div class="form-row">
      ...
      <div class="form-row">
        ...
        <input type="hidden" name="order_id" id="order_id" value="<?php echo 'order_'. uniqid(); ?>">
      </div>
...
<input type="hidden" id="billing_discount" name="billing_discount" value="">

      <br>

      <button type="reset" class="btn btn-home"><a href="index.php" style="text-decoration: none; color: white;">Cancel</a></button>
      &nbsp;&nbsp;&nbsp;
    <button type="submit" name="btnbook" id="payNow" class="btn btn-register" style="color: white;">Pay Now</button>
