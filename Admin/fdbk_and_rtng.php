...
<main id="main" class="main">
    ...
    <div class="row">
        <div class="col">
            <h2 class="text-center mt-4 mb-3">Feedback and Rating Lists</h2>
    ...
    <table class="table table-bordered table-striped">
        ...
        <tbody>
            ...
            $offset = ($page - 1) * $limit; // Offset for records

            // SQL query to fetch data with pagination
            $sql = "SELECT u_id, u_email, u_feedback, u_rate, approval_status FROM e_rate_or_feedback LIMIT $offset, $limit";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                $counter = $offset + 1; // Initialize counter variable
                // Output data of each row
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>";
                    // Display the counter instead of u_id
                    echo "<td style='text-align: center;'>" . $counter . "</td>";
                    echo "<td style='text-align: center;'>" . $row["u_email"] . "</td>";
                    echo "<td style='text-align: center;'>" . $row["u_feedback"] . "</td>";
                    echo "<td style='text-align: center;'>" . $row["u_rate"] . "</td>";
                    echo "<td style='text-align: center;'>" . $row["approval_status"] . "</td>";
                    echo "<td style='text-align: center;'>
                            <a href='UD files/user-fdbk-rtng-data-update.php?id=" . $row["u_id"] . "' class='btn btn-sm btn-primary' onclick='return confirm(\"Are you sure you want to update?\");'>Update</a>
                            <a href='UD files/user-fdbk-rtng-data-delete.php?id=" . $row["u_id"] . "' class='btn btn-sm btn-danger' onclick='return confirm(\"Are you sure you want to delete?\");'>Delete</a>
                          </td>";
...
<div class="text-center" style="padding-left: 40%;">
    <nav aria-label="Page navigation">
        <ul class="pagination">
            ...
            // Previous button
            $prev = $page - 1;
            echo "<li class='page-item'><a class='page-link' href='?page=$prev' aria-label='Previous'>Previous</a></li>";

            // Pagination numbers
            require "connection.php";
            $sql = "SELECT COUNT(*) AS total FROM e_rate_or_feedback";
            $result = $conn->query($sql);
            ...
            // Next button
            $next = $page + 1;
            echo "<li class='page-item'><a class='page-link' href='?page=$next' aria-label='Next'>Next</a></li>";
            ?>
        </ul>
    </nav>
</div>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function filterRows(input) {
      ...
      allRows.forEach(row => {
        ...
        const approvalStatus = row.querySelector('td:nth-child(5)').textContent.toLowerCase().trim();

        if (email.includes(filterValue) || approvalStatus.includes(filterValue)) {
