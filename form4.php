<?php
   ...
   //Alert ...In this code ... I make a javascript and payment both success.
        ...
        <script>
            ...
            function validateForm() {
              ...
              function initiateRazorpayPayment() {
                  ...
                  // var payAmount = parseFloat($('#tprice').val()) || 0;
                  ...
                  $('#payNow').click(function(e) {
                      ...
                      var billing_mobile = $('#usercontact').val();
                      var billing_email = $('#useremail').val();
                      var payAmount = parseFloat($('#tprice').val()/2) || 0; // Corrected to use total price
              ...
              function validateFormAndInitiatePayment(event) {
                  event.preventDefault(); // Prevent form submission
                  if (validateForm()) {
                      initiateRazorpayPayment(); // If form validation succeeds, initiate Razorpay payment process
                  }
                  return false;
              }
...
<form id="payForm" action="" method="POST" onsubmit="validateFormAndInitiatePayment(event)">
      <div class="form-row">
...
<div class="form-row">
  ...
  <div class="form-group col-xl-4">
    ...
    <input type="text" name="tprice" style="background: transparent; color: #fff;" class="form-control glow-effect" id="tprice" readonly>
      ...
      &nbsp;&nbsp;&nbsp;
    <button type="submit" id="payNow" class="btn btn-register" style="color: white;">Pay Now</button>
...
<!-- Payment Process Script Written By Brij -->
...
<!-- <script>
  jQuery(document).ready(function($) {
    $('#PayNow').click(function(e) {
      ...
      var paymentOption = '';
      let billing_name = $('#ufname').val();
      let billing_mobile = $('#usercontact').val();
      let billing_email = $('#useremail').val();
      var shipping_name = $('#ufname').val();
      var shipping_mobile = $('#usercontact').val();
      var shipping_email = $('#useremail').val();
      var paymentOption = "netbanking";
      var dividedByTwo = $('#tprice').val();
      var payAmount = parseFloat(dividedByTwo) / 2;
      ...
      $.ajax({
        ...
        if (data.res == 'success') {
          ...
          var options = {
            ...
            "handler": function(response) {
              window.location.replace("http://localhost:6317/PHP_PROGRAMS/Event%20Organisers/form.php/payment-success.php?oid=" + orderID + "&rp_payment_id=" + response.razorpay_payment_id + "&rp_signature=" + response.razorpay_signature);
            },
            "modal": {
              "ondismiss": function() {
                window.location.replace("http://localhost:6317/PHP_PROGRAMS/Event%20Organisers/form.php/payment-success.php?oid=" + orderID);
              }
            },
            "prefill": {
          ...
          rzp1.on('payment.failed', function(response) {
            window.location.replace("http://localhost:6317/PHP_PROGRAMS/Event%20Organisers/form.php/payment-failed.php?oid=" + orderID + "&reason=" + response.error.description + "&paymentid=" + response.error.metadata.payment_id);
          });
          rzp1.open();
        }
      });
 ...
 <!-- Payment Process Script Written By Anant -->
...
//         var billing_mobile = $('#usercontact').val();
//         var billing_email = $('#useremail').val();
//         var payAmount = parseFloat($('#tprice').val()) || 0; // Corrected to use total price
